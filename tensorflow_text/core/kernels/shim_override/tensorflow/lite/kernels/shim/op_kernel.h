...
absl::Status GetAttr(const std::string& attr_name,
  ...
  // Manual unwrap since std::get might throw if variant setup is complex on old c++?
  if (auto* v = std::get_if<AttrType>(&attr_value)) {
       *value = *v;
       return absl::OkStatus();
  }
  return absl::InternalError("Attribute type mismatch");
}
}  // namespace internal
